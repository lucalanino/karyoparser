...
#' Base (Disease-Agnostic) Cytogenetic Parsing Rules
#'
#' A `karyo_rules` tibble of general, disease-agnostic regex rules for
#' structural aberrations -- dicentrics, rings, insertions, duplications,
#' triplications, markers, derivatives, and generic
#' translocations/additions/inversions/deletions. These apply to any karyotype
#' regardless of disease and form the shared foundation that disease-specific
#' panels extend: [myeloid_rules] is `base_rules` plus myeloid-specific
#' recurrent lesions. Build a new panel by binding your own rows onto
#' `base_rules` and passing the result through [validate_rules()].
#'
#' @format A `karyo_rules` tibble with columns: `flag_name`, `regex`,
#'   `category`, `priority`, `counts_for_monosomal`, `competition_group`.
#' @seealso [myeloid_rules], [validate_rules()]
#' @export
base_rules <- validate_rules(tibble::tribble(
...
#' aberrations relevant to myeloid neoplasms. It is composed of the shared
#' [base_rules] (general structural aberrations) plus myeloid-specific recurrent
...
#' @format A `karyo_rules` tibble with columns: `flag_name`, `regex`,
#'   `category`, `priority`, `counts_for_monosomal`, `competition_group`.
#' @seealso [base_rules], [validate_rules()]
#' @export
myeloid_rules <- validate_rules(dplyr::bind_rows(
  tibble::tribble(
    ~flag_name          , ~regex                                                                        , ~category             , ~priority , ~counts_for_monosomal , ~competition_group ,
