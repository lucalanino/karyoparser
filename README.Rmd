...
- **Partial gain from unbalanced der's**: only the partial loss is flagged (`unbal_partial_loss_*`).
- **Three-way translocations**: only trip the generic `general_translocation` flag; partial-loss derivation is limited to two-partner `der(a)t(a;b)`.
- **Non-ISCN inputs**: array CGH/SNP `seq[GRCh38]` notation and standalone FISH results aren't parsed.

## Learn More

- `vignette("karyoparser", package = "karyoparser")` -- the full
