...
- **Multi-partner (three-way) translocations**: A complete `t(a;b;c)` matches only the generic `general_translocation` flag (no specific-breakpoint flag) and is assumed balanced; a `der()` of a three-way fires only `derivative_chromosome` (no balanced/unbalanced or partial-loss derivation). Both still count toward `complex_karyotype`/`monosomal_karyotype`.
