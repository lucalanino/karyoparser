...
- **[P1] R/ reorganization**: One file per export (roxygen on top); split helpers.R into cohesive groups (`input.R`, `tokens.R`, etc.); consolidate shared check/preprocess internals.
- **[P1] Output schema width knobs**: The full output schema is large and only fully useful in select cases. Offer a way to return a trimmed vs. full column set (some knob(s) on `parse_karyo()`). Design TBD -- ideas to discuss.
