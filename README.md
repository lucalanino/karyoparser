...
`t_9_11_p21_q23`, the family flag `t_v_11q23`, and
`general_translocation`; `del(5q)` sets both `del_5q` and
`general_deletion`.
...
replaced by `_` (e.g. `t(X;18)(p11;q11)` becomes `t_X_18_p11_q11`). This
keeps output columns free of characters (`(`, `)`, `;`) that would
otherwise force backtick-quoting (`` r$`t(X;18)(p11;q11)` ``) in
interactive use. `validate_rules()` also errors if two distinct
`flag_name` values sanitize to the same column name.
